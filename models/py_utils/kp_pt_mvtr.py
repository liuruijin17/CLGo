...
from .transformer import build_transformer, build_transformer_decoder
...
class kp(nn.Module):
    def __init__(self,
        ...
        self.tv_transformer = build_transformer_decoder(hidden_dim=hidden_dim,
                                                        dropout=drop_out,
                                                        n_heads=num_heads,
                                                        dim_feedforward=dim_feedforward,
                                                        dec_layers=dec_layers,
        ...
        self.mv_tr_dec = build_transformer_decoder(hidden_dim=hidden_dim,
                                                   dropout=drop_out,
                                                   n_heads=num_heads,
                                                   dim_feedforward=dim_feedforward,
                                                   dec_layers=1,
        ...
        size_top = torch.Size([self.db.batch_size, np.int(7), np.int(4)])
        # print(size_top);exit()
    ...
    def _parallel(self, *xs, **kwargs):
        ...
        # pv_tgt        = torch.zeros_like(self.query_embed.weight)
        # pv_tgt_posemb = self.query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1)
        hs, enc_mem = self.transformer(self.input_proj(p), pmasks, self.query_embed.weight, pos)  # nheads B nqueries hdim
        ...
        p = F.grid_sample(enc_mem, grid)
        pmasks = F.interpolate(masks[:, 0, :, :][None], size=p.shape[-2:]).to(torch.bool)[0]
        pos = self.tv_position_embedding(p, pmasks)
        tv_tgt = torch.zeros_like(self.tv_query_embed.weight)
        tv_tgt_posemb = self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1)
        tv_mem = rearrange('b c h w -> (h w) b c', p)
        tv_mem_mask = pmasks
        tv_mem_posemb = pos
        hs = self.tv_transformer(
            torch.zeros_like(self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1)),
            rearrange(p, 'b c h w -> (h w) b c'),
            memory_key_padding_mask=pmasks.flatten(1),
            pos=pos.flatten(2).permute(2, 0, 1),
            query_pos=self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1))  # nheads B nqueries hdim
        hs = hs.transpose(1, 2)
    ...
    def _sequential(self, *xs, **kwargs):
        ...
        pmasks = F.interpolate(masks[:, 0, :, :][None], size=p.shape[-2:]).to(torch.bool)[0]
        pos    = self.tv_position_embedding(p, pmasks)
        tv_tgt        = torch.zeros_like(self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1))
        tv_tgt_posemb = self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1)
        tv_mem        = rearrange(p, 'b c h w -> (h w) b c')
        tv_mem_mask   = pmasks.flatten(1)
        tv_mem_posemb = pos.flatten(2).permute(2, 0, 1)
        # print('tv_tgt.shape: {}'.format(tv_tgt.shape))  # nq b c
        # print('tv_tgt_posemb.shape: {}'.format(tv_tgt_posemb.shape)) # nq b c
        # print('tv_mem.shape: {}'.format(tv_mem.shape))  # hw b c
        # print('tv_mem_mask.shape: {}'.format(tv_mem_mask.shape))  # b hw
        # print('tv_mem_posemb.shape: {}'.format(tv_mem_posemb.shape))  # hw b c
        # exit()
        mv_ = self.mv_tr_dec(tv_mem,
                             rearrange(enc_mem, 'b c h w -> (h w) b c'),
                             memory_key_padding_mask=pv_pmasks.flatten(1),
                             pos=pv_pos.flatten(2).permute(2, 0, 1),
                             query_pos=tv_mem_posemb)

        hs = self.tv_transformer(torch.zeros_like(self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1)),
                                 mv_,
                                 memory_key_padding_mask=pmasks.flatten(1),
                                 pos=pos.flatten(2).permute(2, 0, 1),
                                 query_pos=self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1))  # nheads B nqueries hdim
