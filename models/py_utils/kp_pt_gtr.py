...
class kp(nn.Module):
    ...
    def _parallel(self, *xs, **kwargs):
        ...
        # print(p.shape);exit()
        # x_proj = F.grid_sample(images, grid)  # 208 128
        p = F.grid_sample(enc_mem, grid)
        # p = self.tv_conv1(x_proj)
        # p = self.tv_bn1(p)
        # p = self.tv_relu(p)
        # p = self.tv_maxpool(p)  # 52 32
        # p = self.tv1(p)  # 52 32
        # p = self.tv2(p)  # 26 16
        # p = self.tv3(p)  # 13 8
        # p = self.tv4(p)  # 7 4
        ...
        # tgt memory mask pos_embed query_embed
        hs = self.tv_transformer(torch.zeros_like(self.tv_query_embed.weight),
                                 rearrange('b c h w -> (h w) b c', p),
                                 pmasks,
                                 pos,
                                 self.tv_query_embed.weight.unsqueeze(1).repeat(1, self.db.batch_size, 1))  # nheads B nqueries hdim
    ...
    def _sequential(self, *xs, **kwargs):
        ...
        # print(p.shape);exit()
        # x_proj = F.grid_sample(images, grid)  # 208 128
        # print('enc_mem.shape: {}'.format(enc_mem.shape))
        p = F.grid_sample(enc_mem, grid)
        pmasks = F.interpolate(masks[:, 0, :, :][None], size=p.shape[-2:]).to(torch.bool)[0]
        pos    = self.tv_position_embedding(p, pmasks)
        # print('pmasks.shape: {}'.format(pmasks.shape))
